...
startServer().then(s => {
  server = s;
  console.error("Kaia MCP Server initialized successfully");
...
app.get("/", (req: Request, res: Response) => {
  res.json({
    name: "Kaia MCP Server",
    version: "1.0.0",
    description: "MCP Server for Kaia blockchain interactions",
    endpoints: {
      api: "/api",
      health: "/health"
    }
...
app.listen(PORT, HOST, () => {
  console.error(`Kaia MCP Server listening on ${HOST}:${PORT}`);
}).on('error', (error: any) => {
